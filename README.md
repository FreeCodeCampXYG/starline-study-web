...
把客户提供的 TXT、PDF、PPT/PPTX、公开视频链接或它们的组合，整理成一份按用户任务进入、有来源定位、阅读进度和行动清单的单文件 HTML 学习笔记；首版生成后再做一次真实用户任务导向的 UX 优化，需要压缩 PDF 时同步保留或重建章节书签。
...
首版网页不会直接作为成品交付。Skill 会从“开始学习、继续上次、搜索定位、理解重点、回到证据”等高频任务重新检查首屏、主行动、信息密度、渐进披露、可访问性和移动端体验，然后修正具体可用性问题。对于实质设计任务，联网可用时会核对当前 Apple 官方设计指南、Kimi 公开界面和可信设计 Skill/开源先例；只吸收清晰、内容优先、安静工作区和熟悉控件等原则，不复刻品牌外观。
