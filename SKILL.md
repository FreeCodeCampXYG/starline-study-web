...
8. 首版网页生成后不得直接交付，执行一次[生成后 UX 优化门禁](references/visual-ux-contract.md#生成后-ux-优化门禁)：从学习者最常见的“开始学习、继续上次、搜索定位、理解重点、回到证据”任务反推首屏与导航，审计选择负担、主行动、信息密度、阅读路线、交互反馈和移动端。对于实质性设计或用户明确要求高质量体验的任务，在联网可用时核对当前 Apple 官方设计指南、Kimi 公开产品界面及 1–3 个可信设计 Skill 或维护活跃的开源先例，记录 `keep / adapt / reject`；只吸收清晰、内容优先、安静工作区、熟悉控件、自适应和渐进披露等原则，不复制品牌标识、商业外观或具体页面。列出首版的具体可用性问题，完成修正后再进入最终校验。
9. 校验网页：文件存在、无 `javascript:` URL、图片路径可读、导航与移动端布局可用、所有关键结论有来源定位、无证据限制清楚可见。使用真实设备视口验证横向溢出、任务筛选、弹层打开及各关闭路径、阅读进度和收藏持久化；检查页面自身脚本无递归或未捕获异常，并按 [交互与体验契约](references/visual-ux-contract.md) 区分网页错误与浏览器扩展错误。命令行窗口尺寸截图不能代替设备视口。必要时修复后再交付。
...
- 无法联网做设计先例研究：继续使用本地交互契约完成生成后 UX 优化，并把“外部先例未核对”标为 `missing evidence`；不得因此跳过用户任务审计，也不得把记忆中的旧产品界面写成当前事实。
...
5. 生成后 UX 优化 gate：首屏首先服务学习而非展示主题、品牌或统计；只有一个主行动；搜索、继续学习和证据回溯容易发现；样式设置、来源权利和高级工具按任务频率渐进披露；Apple/Kimi 仅作原则校准，不作品牌复刻。
6. 页面 gate：本地打开不报错，响应式布局可读，脚本不依赖秘密或远程执行。
7. 公开发布网页 gate：仅在 GitHub Pages 或其他公开站点交付时启用；明确区分代码署名、内容作者与素材权利，确保代码许可不自动覆盖课件图片、文字摘录、整理数据、二维码或第三方标识，并完成所有弹层关闭路径和页面控制台回归。
...
- 学习型长文档默认提供任务入口、核心阅读线和可见进度；阅读状态只在本地保存，不得静默上传。
- 最终网页必须经过一次生成后 UX 优化：交付摘要列出首版发现的 3–6 个具体可用性问题、实际修正和已完成的浏览器检查；未完成的外部先例研究、跨浏览器检查或真人评审必须标为 `missing evidence`，不得笼统宣称“已达到 Apple/Kimi 水准”。
