...
把 TXT、PPT/PPTX 和公开视频链接转成可追溯、图文并茂、可直接打开的学习笔记网页。
...
1. 明确受众、输出语言、网页标题、深度和保存位置；用户未指定时，沿用来源语言，面向一般学习者，输出到工作区内独立目录。
2. 建立输入清单，记录每个来源的类型、可访问性、更新时间或页码/时间戳能力。只在缺少输入本身或关键访问权限时暂停。
3. 对本地 TXT/PPTX 运行 `scripts/extract_local_sources.py`。阅读生成的 source packet，并按 [输入分析规则](references/input-analysis.md) 处理视频链接与失败降级。生成数据只能记录文件名或相对于交付目录的 POSIX 路径，不得写入用户主目录或其他绝对路径。
4. 先逐来源提炼，再跨来源综合。为每个重要结论保留 `[Sx:定位]`；区分来源事实、合理推断和学习建议。冲突并列呈现，不强行融合。
5. 按学习目标组织内容：快速摘要、知识结构、核心章节、例子/类比、易错点、术语、主动回忆题和行动清单。遵循 [内容与视觉契约](references/content-contract.md)。
...
- 交付一个独立目录，至少包含 `index.html`、生成页面所用的 JSON、`assets/`（如有）和 `source-packet.json`。
...
```powershell
python scripts/extract_local_sources.py material.txt deck.pptx --output source-packet.json --asset-dir assets
